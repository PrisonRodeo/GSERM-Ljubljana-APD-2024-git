...
#
# setwd("~/Dropbox (Personal)/GSERM/Ljubljana 2024/Notes and Slides")
#
...
tiny<-read_table("https://raw.githubusercontent.com/PrisonRodeo/GSERM-Ljubljana-APD-2024-git/master/Data/tinyTSCSexample.txt")
tiny
...
SCDBVotes<-read_csv("https://raw.githubusercontent.com/PrisonRodeo/GSERM-Ljubljana-APD-2024-git/master/Data/SCDB-2020.csv")
...
SegalCover<-read_csv("https://raw.githubusercontent.com/PrisonRodeo/GSERM-Ljubljana-APD-2024-git/master/Data/Segal-Cover.csv")
...
# Quick regressions: Liberal percentage vs.
# justice ideology...
#
# Overall / pooled regression:

fit<-lm(LiberalPct~Ideology,data=SCData)
summary(fit)

# 1968 only:

fit68<-lm(LiberalPct~Ideology,data=SCData[SCData$term==1968,])
summary(fit68)

# Now repeat for every term in the data:

intercepts<-numeric(length(unique(SCData$term)))
slopes<-numeric(length(unique(SCData$term)))
sees<-numeric(length(unique(SCData$term)))

start<-min(SCData$term)
end<-max(SCData$term)

for(i in start:end) {
  reg<-lm(lm(LiberalPct~Ideology,data=SCData[SCData$term==i,]))
...
pdf("SCOTUS-Beta0s.pdf",7,6)
par(mar=c(4,4,2,2))
plot(Term,intercepts,t="l",lwd=1.5,
     ylab="Estimated Intercept")
abline(h=fit$coefficients[1],lty=2)
dev.off()

pdf("SCOTUS-Beta1s.pdf",7,6)
par(mar=c(4,4,2,2))
plot(Term,slopes,t="l",lwd=1.5,
     ylab="Estimated Slope")
abline(h=fit$coefficients[2],lty=2)
dev.off()

pdf("SCOTUS-SEEs.pdf",7,6)
par(mar=c(4,4,2,2))
plot(Term,sees,t="l",lwd=1.5,
     ylab="Estimated SEE")
abline(h=sqrt(anova(fit)$`Mean Sq`[2]),lty=2)
dev.off()

# Now by justice; let's pick out (say) 
# Justice Sandra Day O'Connor:

fitSDO<-lm(LiberalPct~Ideology,data=SCData[SCData$justiceName=="SDOConnor",])
summary(fitSDO)
